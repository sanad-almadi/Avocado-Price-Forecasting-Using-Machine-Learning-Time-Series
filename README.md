...
The dataset includes detailed order-level information such as date, product size, product type (Conventional or Organic), quantity sold, region, and price per case. A complete data science pipeline was applied, starting with data cleaning, handling missing values, and standardizing data types, followed by feature engineering by extracting time-based features (week, month, year), creating lag features, and computing rolling averages to capture temporal changes.
